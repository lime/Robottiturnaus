...
public class TestiPeruuttavaRobotti extends Robotti {
	...
	int paataSuunta() {
    	...
    	if(jokaSuuntaanOllaanMenty(this.nykyinenSijainti) && historia.size() > 1) {
    		
    		this.historia = this.optiomoiHistoriaa();
    		
			Point viimeSijainti = historia.pollLast();
    		while(viimeSijainti.equals(this.nykyinenSijainti) && !historia.isEmpty()) {
    			if(historia.peekLast() != null)
    			viimeSijainti = historia.pollLast(); //ettei ole nykyinen -hotfix
    		}
    		
    		/* tähän se että jos ottaisi lyhyemmän tien
    		 * hmmm...
    		 */
    		//List<Point> histoCopy = new LinkedList<Point>(historia);
    		Iterator<Point> histoIter = historia.descendingIterator();
    		while( histoIter.hasNext() ) {
    			Point seuraavaHist = histoIter.next();
    			 if (naapuritPisteessa(this.nykyinenSijainti).contains(seuraavaHist) ){
    				 return suuntaNaapuriin(nykyinenSijainti, seuraavaHist);
    			 }
    		}
	...
	boolean onToimivaHistoria(Deque<Point> histArg) {
		LinkedList<Point> historiikki = new LinkedList<Point>(histArg);
		for(int i = 0; i < historiikki.size() - 1; i++) {
			if(!naapuritPisteessa(historiikki.get(i)).contains(historiikki.get(i+1))){
				return false;
			}
		}
		return true;
	...
	private ArrayDeque<Point> optiomoiHistoriaa() {
		Iterator<Point> histoIter = historia.descendingIterator();
		LinkedList<Point> histamiini = new LinkedList<Point>(historia);
		while( histoIter.hasNext() ) {
			Point seuraavaHist = histoIter.next();
			 if (naapuritPisteessa(this.nykyinenSijainti).contains(seuraavaHist) ){
				for(int i = histamiini.lastIndexOf(seuraavaHist) + 1; i < histamiini.size(); i++){
						histamiini.remove(i);
				}
			 }
		}
		if(onToimivaHistoria(histamiini)) {
			return new ArrayDeque<Point>(histamiini);
			//return histamiini;
		} else {
			System.err.print("histamiini ei ole hyvä");
			//return new LinkedList<Point>(historia);
			return historia;
		}
	}
}
